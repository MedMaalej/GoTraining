 ...
 * @author acer
 */
class configuirationmembre extends CI_Controller{
    //put your code here
  ...
  function __construct() {
	...
	function manage(){
		...
		// Last_Name will be converted into Last Name using humanize() function, under inflector helper of the CI core.
             ...
             $where=array('for_table'=>'membre','section'=>'membre_list.php');
		$this->data['results'] = $this->codegen_model->getPrametre('prametrechamp','nom_champ,for_table,order,etat,visiblite,label,filter',$where,$config['per_page'],$this->uri->segment(3));
	        ...
	        $this->load->view('membre/config', $this->data);
    ...
    function listechamp()
    {
        ...
        $this->data['liste']=$this->codegen_model->getChampAjouter('prametrechamp','nom_champ',$where);
          $this->load->view('membre/suprimerchamp',$this->data);
    }
    function supprimerchamp()
    {
        $where=array('for_table'=>'membre','section'=>'membre_add.php','nom_champ'=>$_GET['nom_champ']);
        $where1=array('for_table'=>'membre','section'=>'membre_list.php','nom_champ'=>$_GET['nom_champ']);
        if(($this->codegen_model->deletebychamp('prametrechamp',$where)==true)&&($this->codegen_model->deletebychamp('prametrechamp',$where1)==true))
        {
            $this->db->query('ALTER TABLE membre DROP COLUMN '.$_GET['nom_champ'].';');
            redirect(base_url().'index.php/membre/configuirationmembre/listechamp');
        }
      ...
      function ajoutechamp()
    {
        ...
        $name=$_POST['labelchamp'];
           ...
           $this->db->query('ALTER TABLE membre ADD COLUMN '.$name.'  '. $typechamps.';');
           $where=array('for_table'=>'membre','section'=>'membre_add.php');
           $where1=array('for_table'=>'membre','section'=>'membre_list.php');
           $num=$this->codegen_model->countTablebyparametre($where);
            $num1=$this->codegen_model->countTablebyparametre($where1);
           $data=array('nom_champ'=> $name,'for_table'=>'membre','section'=>'membre_add.php','filter'=>'0','order'=>$num+1 ,'etat'=>'0','visiblite'=>'1','label'=> $name,'type'=> $zone);
            $data1=array('nom_champ'=> $name,'for_table'=>'membre','section'=>'membre_list.php','filter'=>'0','order'=>$num1+1 ,'etat'=>'0','visiblite'=>'1','label'=> $name,'type'=> $zone);
         if (($this->codegen_model->add('prametrechamp',$data) == TRUE)&&($this->codegen_model->add('prametrechamp',$data1) == TRUE))
         {
             redirect(base_url().'index.php/membre/configuirationmembre');
         }
	...
	$this->load->helper('url');	
			...
			if (($this->codegen_model->editby('prametrechamp',$data,$where) == TRUE)&&($this->codegen_model->editby('prametrechamp',$data1,$where1) == TRUE))
			{
				...
				redirect(base_url().'index.php/membre/configuirationmembre?nomtable='.$_POST['nomtable']);
			}
